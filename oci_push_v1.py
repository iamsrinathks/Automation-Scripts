...
def download_crane():
    ...
    arch = "amd64" if platform.architecture()[0] == "64bit" else "386"

    # Construct the download URL for the crane binary
    crane_url = f"https://github.com/google/go-containerregistry/releases/latest/download/crane_{os_name}_{arch}"
    crane_path = "/usr/local/bin/crane"  # Change this if needed

    # Download crane binary
    urllib.request.urlretrieve(crane_url, crane_path)
